 ...
 * 从数组中选50个元素
 * */
var my_sample = function(source_array, collec_point_num, token) {

    //先随机从数组中减去10个点
    var s_length = source_array.length;
    var mySet = randomNum(0, s_length, s_length-Math.round(Math.random()*50));
    var my_set_size = mySet.size();
    var my_set_value = mySet.values();
    console.log(my_set_value+'')
    var random_array = [];
    for(var i = 0; i < my_set_size; i++){
        random_array.push(source_array[my_set_value[i]]);
    }

    var n = random_array.length;
    for (var res = [], r = 0, o = 0; o < n; o++){
        // o >= r * (n - 1) / (t - 1) && (i.push(e[o]), r += 1);
        o >= r * (n - 1) / (collec_point_num - 1) && (res.push(my_n(token, random_array[o]+"")), r += 1);
    }
    return res
}
