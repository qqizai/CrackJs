...
function _my_encrypt(traceDataArray, token, position_left) {
    var affter_array = []
    for (var i of traceDataArray) {
        affter_array.push(n(token, i + ""));
    };
    var _sample = function(e, t) {
        var t_n = affter_array.length;
        if (t_n <= t)
            return e;
        for (var i = [], r = 0, o = 0; o < t_n; o++)
            o >= r * (t_n - 1) / (t - 1) && (i.push(affter_array[o]), r += 1);
        return i
    }

    var _n = _sample(affter_array, 50);
    var value_p = n(token, parseInt(position_left, 10) / 320 * 100 + "");

    var data = JSON.stringify({
        d: B(_n.join(":")),
        m: "",
        p: B(value_p),
        ext: B(n(token, 1 + "," + traceDataArray.length))
    });
    return data;
}
