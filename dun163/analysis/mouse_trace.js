...
function my_encrypt(traceDataArray, token, position_left) {
    ...
    var _sample = function(e, t) {
        ...
        for (var i = [], r = 0, o = 0; o < n; o++)
            o >= r * (n - 1) / (t - 1) && (i.push(my_n(token, e[o]+"")), r += 1);
        return i
    }

    var _n = _sample(traceDataArray, 50);
    var value_parseInt = parseInt(position_left, 10) / 3;
    var value_p = my_p(token, value_parseInt + "");
    var r = my_h(value_p);
    var value_d = my_h(_n.join(":"));
    var value_ext = my_h(my_p(token, 1 + "," + traceDataArray.length));
